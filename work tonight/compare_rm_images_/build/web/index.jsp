...
props.load(in);
String url = props.get("geturl").toString();
 %>

<script type="text/javascript">
var URLs, result, cpsHeight, cpsWidth, cfHeight, cfWidth, readindex = 0,count = 0;
var cpsurls, cpsbytes, cfurls, cfbytes;
...
function readFromResult(){
    cpsurls=[];cpsbytes=[];cfurls=[];cfbytes=[];  
    for(i= readindex;i<=(readindex+ 10);i++){
        cpsurls.push(URLs.result.CPS[i].URL);
        cpsbytes.push(URLs.result.CPS[i].URL);
        cfurls.push(URLs.result.CF[i].URL);
        cfbytes.push(URLs.result.CF[i].URL);
    } 
    console.log(cpsurls+" "+cpsurls+" "+cfurls+" "+cfbytes);
...
<section>
	<div class="col-lg-12">
		...
		<div class="col-lg-6"><button  id="buttons" onclick="readFromResult()">Next ></button></div>
	</div>
<section>

    <!--<footer class="fixed-bottom"><p id="info">NB: Click Previous and then Next if multiple images load at once. This is due to delays in loading images from the server. </p></footer>-->
</body>
